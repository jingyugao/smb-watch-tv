...
App 启动后会自动检查 GitHub Releases，也可以在主界面选择“检查更新”手动检查。
发现新版本后，确认即可下载；APK 通过 Release 附带的 SHA-256 校验后，会打开电视的系统安装器。

- Stable 版本只接收正式版本
- Dev/Beta 版本同时接收测试版本和正式版本
- 首次应用内更新时，需要允许“SMB追剧”安装未知应用
- Android TV 会在安装前要求用户确认，普通应用无法静默安装
- 更新必须继续使用相同包名和签名证书，否则无法覆盖安装
